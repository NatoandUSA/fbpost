...
from adapters.facebook_publication import scan_post_permalink, copy_post_permalink, has_pending_notice, search_group_post, search_group_my_posted, search_group_pending, resolve_share_reference
...
def reconcile_existing_post(target_url, content, account_id=None, gpm_api_url=None):
    ...
    try:
        with sync_playwright() as p:
            ...
            # Prefer Facebook's profile-scoped "Your content" surfaces over feed
            # ranking. Published content must still yield a concrete canonical Group
            # permalink; pending content is terminal only as pending.
            if "/groups/" in target_url:
                permalink = search_group_my_posted(page, target=target_url, content=content)
                if permalink:
                    record_posted_link(
                        target_url, permalink, content, note="Đã xuất bản (my-posted-content đối soát)",
                        account_id=account_id or "default", url_type="post", publish_state="published"
                    )
                    return ActionResult(True, "RECONCILE_PUBLISHED", "Đã xác minh bài trong Nội dung của bạn / Đã đăng.",
                                        state="published", target_url=target_url, result_url=permalink, url_type="post",
                                        metadata={"evidence_source": "facebook_my_posted_content"})
                if search_group_pending(page, target=target_url, content=content):
                    record_posted_link(
                    ...
                    return ActionResult(True, "RECONCILE_PENDING", "Đã xác minh bài trong hàng chờ duyệt.",
                                        state="pending", target_url=target_url, url_type="group",
