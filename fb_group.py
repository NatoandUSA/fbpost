...
def post_to_group(group_url, content, image_path=None):
    ...
    with sync_playwright() as p:
        try:
            ...
            # Random mouse movements before navigation to simulate human
            ...
            if image_path and os.path.exists(image_path):
                ...
                try:
                    # Facebook uses hidden input[type=file] for photo uploads
                    # We can use set_input_files on the generic file input
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python fb_group.py <GROUP_URL> '<YOUR POST CONTENT>' [IMAGE_PATH]")
        sys.exit(1)
    
    group_url = sys.argv[1]
    content = sys.argv[2]
    image_path = sys.argv[3] if len(sys.argv) > 3 else None
    post_to_group(group_url, content, image_path)
