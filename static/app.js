document.addEventListener('DOMContentLoaded', () => {
    ...
    if (refreshGpmBtn) {
        refreshGpmBtn.addEventListener('click', async () => {
            showToast('Đang quét và đồng bộ profile từ GPM Login...');
            await loadAccounts();
            showToast('Đã cập nhật danh sách Profile GPM!');
        });
    }

    if (syncGpmTableBtn) {
        syncGpmTableBtn.addEventListener('click', async () => {
            showToast('Đang tải danh sách profile từ GPM Login...');
            await loadAccounts();
            showToast('Đã đồng bộ danh sách GPM vào bảng!');
        });
    }

    if (openSelectedProfileBtn) {
        openSelectedProfileBtn.addEventListener('click', async () => {
            const accId = accountSelector ? accountSelector.value : '';
            if (!accId || accId === '__rotate__') {
    ...
    if (studioGenerateBtn) studioGenerateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const payload = {brand:document.getElementById('studio-brand')?.value || 'umee', topic:document.getElementById('studio-topic')?.value || 'room_sale', keyword, audience:document.getElementById('studio-audience')?.value.trim() || '', angle:document.getElementById('studio-angle')?.value.trim() || '', verifiedFacts:document.getElementById('studio-facts')?.value.trim() || '', tone:document.getElementById('studio-tone')?.value || 'natural', length:document.getElementById('studio-length')?.value || 'medium', cta:document.getElementById('studio-cta')?.value || 'message', apiKeys:keyText.split(/[\r\n,;]+/).map(v=>v.trim()).filter(Boolean)};
...
async function loadGroupRegistry(){
 ...
 try{const r=await fetch('/api/groups/moderation'); if(!r.ok) throw new Error('HTTP '+r.status); const d=await r.json(); const rows=d.groups||[];
 body.innerHTML=rows.length?rows.map(g=>`<tr><td>${safe(g.group_url||'')}</td><td>${g.requires_approval?'Có':'Không'}</td><td>${Number(g.pending_count||0)}</td><td>${g.decision==='skip'?'SKIP ≥2 pending':g.decision==='low_priority'?'Thấp':'Bình thường'}</td><td>${safe(g.last_profile_id||'')}</td></tr>`).join(''):'<tr><td colspan="5">Chưa có group cần quản trị viên duyệt.</td></tr>';}
