...
class V6131RuntimeRecoveryTests(unittest.TestCase):
    ...
    def test_recovery_sources_preserve_fail_closed_contracts(self):
        ...
        self.assertIn('a[role="button"]', group)
        self.assertNotIn("joined evidence=post_composer_capability", group)
        self.assertIn("exact-route-shell-nudge", comment)
        self.assertNotIn("exact-route-comment-dialog", comment)
