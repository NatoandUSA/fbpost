...
class ReconcileRepository(BaseRepository):
    DELAYS=(30,120,600)
    def enqueue(self,target_url,content,account_id=None,queue_item_id=None,delay_seconds=30):
        ...
        with self.transaction() as conn:
            ...
            conn.execute("INSERT INTO reconciliation_queue(id,target_url,content,account_id,queue_item_id,attempt,next_reconcile_at,status,created_at,updated_at) VALUES(?,?,?,?,?,0,?,'pending',?,?)",(rid,target_url,content,account_id,queue_item_id,_iso(now+timedelta(seconds=delay_seconds)),_iso(now),_iso(now)))
        return rid
    ...
    def finish_attempt(self,rid,state,result_url='',error=''):
        now=_now()
        with self.transaction() as conn:
            row=conn.execute("SELECT attempt FROM reconciliation_queue WHERE id=?",(rid,)).fetchone()
            ...
            if state in ('published','pending'):
                ...
                status='pending'; next_at=_iso(now+timedelta(seconds=self.DELAYS[attempt]))
